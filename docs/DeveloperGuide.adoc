...
`AddPhotoCommandParser` will then return a `AddPhotoCommand` to be executed in the `LogicManager`. Upon the `execute()` method of `AddPhotoCommand`, a new `Person` object, `editedPerson` is created with all the same field as the `personToEdit` with the addition of the new `Photo` field. The `editedPerson` will replace `personToEdit` using the `updatePerson()` in the `Model` class. A success message will be generated if the photo addition is successful.

=== Design considerations

Aspect: Implementation of AddPhotoCommand
Alternative 1(current choice): Have AddPhotoCommand implement UndoableCommand
Pros: We will retain the logic flow of how commands are executed. This option increases code reusability and retains the cohesion of each class.
Cons: Logic flow is embedded in multiple layers. New developers might find it hard to understand the complicated logic flow.

---

**Aspect:** Choice of how Image data type is obtained from filepath +
**Alternative 1(current choice):** Using Image object to represent an photo of a contact. +
**Pros:** Works well with JavaFx scenebuilder as Image class can be passed into ImageView class to load the image. +
**Cons:** Might present a problem if our team chooses to use alternative GUI generators, Image class might not be applicable to generate an Image. +
**Alternative 2:** Using a String to represent the filepath of the photo. +
**Pros:** Do not have to go through multiple stages of conversion to represent the photo as an Image object. +
**Cons:** Unable to pass String into ImageView class in Javafx to load an image. Therefore still have to convert into an Image object to pass through to ImageView class. Thus, using Image Object to represent a photo makes more sense.
...
=== FindPhoneNumberCommand mechanism

The `FindPhoneNumberCommand` class is added along with the `FindPhoneNumberCommandParser` class and `NumberContainsKeywordsPredicate` class.
The `FindPhoneNumberCommand` class extends from the `Command` class and hence cannot be undone or redone using the `Undo` or `Redo` commands. +

In the `NumberContainsKeywordsPredicate` class, it implements the `Predicate` class. The purpose of this class is to match
the keyword input from the user with the `Phone` value (phone number of contacts) and the `FindPhoneNumberCommandParser` would return the matched contact
details to the user in the GUI. +

The `AddressBookParser` has to be updated to be able to recognise the new `COMMAND_WORD` for `FindPhoneNumberCommand`. In addition,
we need to create a new `FindPhoneNumberCommandParser` class as well. This parser class implements the `Parser` class and its main function
is to take the input commands and would return a `FindPhoneNumberCommand` object for execution. It would return
an invalid error message if the input is correct and return a successful message  if the inputs  are correct.

==== Design Considerations

**Aspect:** Implementation of `FindPhoneNumberCommand`. +
**Alternative 1 (current choice):** Using a separate command using the command word `findphone` +
**Pros:** Easy to debug and implement. +
**Cons:** Inconvenient for users as they have to use another command. +
**Alternative 2:** Enhancing the current `FindCommand` to allow it to accept phone numbers. +
**Pros:** More convenient for users as they can just a single command word, `find`, to search through the Address Book
with different parameters. +
**Cons:** More difficult to implement and maintain. Testing can be more tedious as well.
